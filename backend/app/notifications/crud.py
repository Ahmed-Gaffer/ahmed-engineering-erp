...
class NotificationCRUD:
    ...
    async def create(self, db: AsyncSession, user_id: int, title: str, message: str | None = None, type: str = "info", link: str | None = None) -> Notification:
        obj = Notification(user_id=user_id, title=title, message=message, type=type, link=link)
        db.add(obj)
        await db.commit()
