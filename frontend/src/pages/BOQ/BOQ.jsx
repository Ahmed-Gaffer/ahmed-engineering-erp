...
import {
  Box, Typography, Card, TextField, Button, Stack, MenuItem, IconButton, Chip,
...
export default function BOQ() {
  ...
  const displayRows = useMemo(() => {
    ...
    rows.forEach(item => {
      if (match(item)) {
        ...
        while (cur) {
          included.add(cur.id);
          cur = cur.parent_id ? idMap[cur.parent_id] : null;
        }
      }
    });
    return rows.filter(item => included.has(item.id));
  }, [rows, search, idMap]);
  ...
  const processedData = useMemo(() => {
    ...
    return displayRows.map(item => {
      ...
      while (cur.parent_id && !visited.has(cur.parent_id)) {
        ...
        const parent = idMap[cur.parent_id];
        if (parent) { path.unshift(parent.id); cur = parent; }
        else break;
      }
  ...
  const getTreeDataPath = useCallback((row) => row._path || [], []);
  ...
  const handleSubmit = async (formData) => {
    ...
    try {
      ...
      if (editItem) {
        await engineeringApi.boqItems.update(editItem.id, payload);
      } else {
    ...
    },
    {
      ...
      renderCell: (params) => (
        <Stack direction="row" spacing={0.5}>
          ...
          <IconButton
            size="small"
            onClick={() => { setDeleteId(params.row.id); setDeleteItemName(params.row.item_code + ' - ' + params.row.description); setDeleteOpen(true); }}
          ...
          <DialogContent>
            <Stack spacing={2} sx={{ mt: 1 }}>
              {!importPreview && !importResult && (
                <Box sx={{ border: '2px dashed', borderColor: 'primary.main', borderRadius: 2, p: 4, textAlign: 'center' }}>
                  <input
                    accept=".xlsx,.xls"
                    style={{ display: 'none' }}
                    ...
                    onChange={async (e) => {
                      ...
                      try {
                        ...
                        if (data.length > 1) {
                          setImportPreview({ headers: data[0], rows: data.slice(1, 11) });
                        } else {
              ...
              {importResult && (
                <>
                  <Alert severity={importResult.errors?.length ? 'warning' : 'success'}>
                    {t('importComplete')}: {importResult.created} {t('items')}
                    {importResult.errors?.length > 0 && `, ${importResult.errors.length} ${t('errors')}`}
                  </Alert>
