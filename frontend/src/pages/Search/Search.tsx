...
const entityMeta = {
  contractors: { icon: <Business />, color: 'secondary.main', labelKey: 'contractors' },
  projects: { icon: <Folder />, color: '#10b981', labelKey: 'projects' },
  employees: { icon: <Person />, color: '#f59e0b', labelKey: 'employees' },
  drawings: { icon: <Engineering />, color: 'secondary.main', labelKey: 'drawings' },
  rfis: { icon: <Engineering />, color: 'secondary.main', labelKey: 'rfis' },
...
const resolveMetaColor = (color, theme) => color === 'secondary.main' ? theme.palette.secondary.main : color;
        ...
        {done && results.length > 0 && Object.entries(grouped).map(([entityType, items]) => {
          ...
          return (
            <motion.div key={entityType} variants={itemVariants}>
              <Card sx={{ borderTop: '4px solid', borderTopColor: 'secondary.main', borderRadius: 2, border: '1px solid', borderColor: 'divider' }}>
                <Stack direction="row" alignItems="center" spacing={1} sx={{ px: 2.5, py: 1.5, borderBottom: '1px solid', borderColor: 'divider' }}>
                  <Avatar sx={{ width: 28, height: 28, bgcolor: resolveMetaColor(meta.color, theme), fontSize: '0.9rem' }}>
                    {meta.icon}
                  </Avatar>
                  <Typography variant="subtitle2" fontWeight={600} flex={1}>{t(meta.labelKey)}</Typography>
                  <Chip label={`${items.length}`} size="small" sx={{ bgcolor: `${resolveMetaColor(meta.color, theme)}18`, color: meta.color, fontWeight: 600 }} />
                </Stack>
                    ...
                    <Box key={item.id}>
                      ...
                      <ListItem disablePadding>
                        <ListItemButton onClick={() => navigate(item.url)} sx={{ px: 2.5, py: 1 }}>
                          <ListItemAvatar>
                            <Avatar sx={{ width: 32, height: 32, bgcolor: `${resolveMetaColor(meta.color, theme)}18`, color: meta.color, fontSize: '0.75rem', fontWeight: 700 }}>
                              {item.code?.charAt(0) || '#'}
                            </Avatar>
                          </ListItemAvatar>
                          <ListItemText
