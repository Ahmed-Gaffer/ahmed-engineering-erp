...
export default function Transmittals() {
  ...
  const handleSubmit = async (formData) => {
    setFormLoading(true);
    try {
      if (editItem) await engineeringApi.transmittals.update(editItem.id, formData);
      else await engineeringApi.transmittals.create({ ...formData, project_id: parseInt(selectedProjectId) });
  ...
  const handleDelete = async (id) => {
    try {
      await engineeringApi.transmittals.delete(id);
      enqueueSnackbar(t('operationSuccess'), { variant: 'success' });
      setDeleteId(null);
      fetchData();
  ...
  const handleSend = async () => {
    try {
      await engineeringApi.transmittals.send(sendDialog.id, { recipient: sendRecipient });
      enqueueSnackbar(t('operationSuccess'), { variant: 'success' });
  ...
  const handleReceive = async () => {
    try {
      await engineeringApi.transmittals.markReceived(receiveDialog.id, { notes: receiveNotes });
      enqueueSnackbar(t('operationSuccess'), { variant: 'success' });
      ...
      <Dialog open={sendDialog.open} onClose={() => setSendDialog({ open: false, id: null })} maxWidth="sm" fullWidth>
        ...
        <DialogContent>
          <TextField autoFocus margin="dense" label={t('recipient')} fullWidth value={sendRecipient}
            onChange={(e) => setSendRecipient(e.target.value)} size="small" sx={{ mt: 1 }} />
        </DialogContent>
        <DialogActions>
      ...
      <Dialog open={receiveDialog.open} onClose={() => setReceiveDialog({ open: false, id: null })} maxWidth="sm" fullWidth>
        ...
        <DialogContent>
          <TextField autoFocus margin="dense" label={t('notes')} fullWidth multiline minRows={3}
            value={receiveNotes} onChange={(e) => setReceiveNotes(e.target.value)} size="small" sx={{ mt: 1 }} />
        </DialogContent>
        <DialogActions>
