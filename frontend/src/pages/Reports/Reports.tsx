                  ...
                  <CardContent>
                    ...
                    <EChart
                      ...
                      option={{
                        ...
                        series: [{
                          type: 'pie',
                          data: progress.projects_by_status.map(d => ({
                            name: t(d.status),
                            value: d.count,
                            ...(STATUS_COLORS[d.status] ? { itemStyle: { color: STATUS_COLORS[d.status] } } : {}),
                          })),
                          radius: ['0%', '70%'],
                          label: { formatter: ({ name, value }) => `${name} (${value})` },
                        }],
                        legend: {},
                      }}
                    />
                  ...
                  <CardContent>
                    ...
                    <EChart
                      ...
                      option={{
                        ...
                        xAxis: { type: 'category', data: financial.ipc_by_status.map(d => t(d.status)) },
                        yAxis: { type: 'value' },
                        series: [{ type: 'bar', data: financial.ipc_by_status.map(d => d.count) }],
                      }}
                    />
                  </CardContent>
                </Card>
            ...
            <Card sx={{ ...sectionSx, overflow: 'hidden' }}>
              <CardContent>
                ...
                <EChart
                  ...
                  option={{
                    tooltip: { trigger: 'axis' },
                    xAxis: { type: 'category', data: progress.projects.map(d => d.code) },
                    yAxis: { type: 'value' },
                    series: [
                      { type: 'bar', name: t('progressPercentage'), data: progress.projects.map(d => d.schedule_progress) },
                      { type: 'bar', name: t('totalIpcPaid'), data: progress.projects.map(d => d.ipc_paid) },
                    ],
                    legend: {},
                  }}
                />
                  ...
                  <Typography variant="subtitle1" fontWeight={600} mb={2}>{t('contracts')} {t('byStatus')}</Typography>
                    <EChart
                      ...
                      option={{
                        ...
                        series: [{
                          type: 'pie',
                          data: financial.contracts_by_status.map(d => ({
                            name: t(d.status),
                            value: d.count,
                            ...(STATUS_COLORS[d.status] ? { itemStyle: { color: STATUS_COLORS[d.status] } } : {}),
                          })),
                  ...
                  <Typography variant="subtitle1" fontWeight={600} mb={2}>{t('workOrders')} {t('byPriority')}</Typography>
                    <EChart
                      ...
                      option={{
                        ...
                        series: [{
                          type: 'pie',
                          data: data.by_priority.map(d => ({
                            name: t(d.priority),
                            value: d.count,
                            ...(STATUS_COLORS[d.priority] ? { itemStyle: { color: STATUS_COLORS[d.priority] } } : {}),
                          })),
                          radius: ['0%', '60%'],
                          label: { formatter: ({ name, value }) => `${name} (${value})` },
                        }],
                ...
                <CardContent>
                  <Typography variant="subtitle1" fontWeight={600} mb={2}>{t('workOrders')} {t('byStatus')}</Typography>
                    <EChart
                      ...
                      option={{
                        tooltip: { trigger: 'axis' },
                        xAxis: { type: 'category', data: data.by_status.map(d => t(d.status)) },
                        yAxis: { type: 'value' },
                        series: [{ type: 'bar', data: data.by_status.map(d => d.count) }],
                      }}
                    />
                </CardContent>
              </Card>
                        ...
                        <Stack direction="row" alignItems="center" spacing={0.5} sx={{ width: '100%', height: '100%' }}>
                          <Box sx={{ flex: 1, height: 6, borderRadius: 3, bgcolor: isDark ? 'rgba(255,255,255,0.1)' : 'rgba(0,0,0,0.06)', overflow: 'hidden' }}>
                            <Box sx={{ width: `${p.value || 0}%`, height: '100%', borderRadius: 3,
                              bgcolor: '#D97706', transition: 'width 0.6s ease' }} />
                          </Box>
              ...
              <>
                <EChart
                  ...
                  option={{
                    ...
                    yAxis: { type: 'value', axisLabel: { formatter: (v) => formatNumber(v) } },
                    series: [
                      { type: 'bar', name: t('contractValue'), data: chartData.map(d => d.contract) },
                      { type: 'bar', name: t('totalBilled'), data: chartData.map(d => d.billed) },
                      { type: 'bar', name: t('totalIpcPaid'), data: chartData.map(d => d.paid) },
                    ],
                    legend: {},
                  }}
                />
