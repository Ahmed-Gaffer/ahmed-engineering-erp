...
import { Add, GroupWork } from '@mui/icons-material';
...
export default function Subcontractors() {
  ...
  useEffect(() => { engineeringApi.projects.list().then((r: any) => setProjects(r.data || [])).catch(() => {}); }, []);
  ...
  const columns = [
    ...
    { field: 'contract_value', headerName: t('contractValue'), type: 'number', width: 140, renderCell: (p: any) => <Typography variant="body2" fontWeight={600}>{formatNumber(p.value)}</Typography> },
  ...
  return (
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }}>
      <Box>
        ...
        <TextField select value={selectedProjectId} onChange={(e: any) => setSelectedProjectId(e.target.value)} label={t('selectProject')} sx={{ mb: 2, minWidth: 280 }}>
