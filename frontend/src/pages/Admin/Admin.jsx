...
import {
  ...
  InputLabel, Chip, Stack, CircularProgress, Alert,
} from '@mui/material';
...
export default function Admin() {
  ...
  const activityColumns = [
    { field: 'username', headerName: t('username'), width: 130 },
    {
      ...
      renderCell: (params) => <Chip label={params.value} size="small" color={params.value === 'create' ? 'success' : params.value === 'delete' ? 'error' : 'primary'} variant="outlined" />,
    },
        ...
        <Paper sx={{ p: 2 }}>
          <Stack direction="row" justifyContent="space-between" alignItems="center" mb={2}>
            <FormControl size="small" sx={{ minWidth: 150 }}>
              ...
              <Select value={actionFilter} label={t('filter')} onChange={(e) => setActionFilter(e.target.value)}>
        ...
        <Paper sx={{ p: 2 }}>
          <Stack direction="row" justifyContent="space-between" alignItems="center" mb={2}>
            <FormControl size="small" sx={{ minWidth: 150 }}>
              ...
              <Select value={logLines} label={t('lines')} onChange={(e) => setLogLines(Number(e.target.value))}>
      ...
      <Dialog open={editDialog.open} onClose={() => setEditDialog({ open: false, key: '', value: '' })} maxWidth="sm" fullWidth>
        ...
        <DialogContent>
          <TextField
            ...
            value={editDialog.value} onChange={(e) => setEditDialog(prev => ({ ...prev, value: e.target.value }))}
          />
        </DialogContent>
        <DialogActions>
      ...
      <Dialog open={createDialog.open} onClose={() => setCreateDialog({ open: false, key: '', value: '', description: '' })} maxWidth="sm" fullWidth>
        ...
        <DialogContent>
          <TextField
            ...
            value={createDialog.key} onChange={(e) => setCreateDialog(prev => ({ ...prev, key: e.target.value }))}
          />
          <TextField
            fullWidth margin="dense" label={t('settingValue')}
            value={createDialog.value} onChange={(e) => setCreateDialog(prev => ({ ...prev, value: e.target.value }))}
          />
          <TextField
            fullWidth margin="dense" label={t('settingDescription')}
            value={createDialog.description} onChange={(e) => setCreateDialog(prev => ({ ...prev, description: e.target.value }))}
          />
        </DialogContent>
        <DialogActions>
