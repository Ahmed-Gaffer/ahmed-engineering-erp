...
const pointTypeColors: Record<string, { bg: string; color: string }> = {
...
export default function Survey() {
  ...
  useEffect(() => {
    engineeringApi.projects.list().then((r: any) => {
      const items = r.data || [];
      setProjects(items);
  ...
  const fetchReadings = useCallback(async (pointId: number) => {
  ...
  const handleReadingSubmit = async (formData: Record<string, any>) => {
  ...
  const pointColumns = [
    ...
    { field: 'northing', headerName: t('northing'), width: 110, renderCell: (params: any) => params.value ?? '-' },
    { field: 'easting', headerName: t('easting'), width: 110, renderCell: (params: any) => params.value ?? '-' },
    { field: 'elevation', headerName: t('elevation'), width: 110, renderCell: (params: any) => params.value ?? '-' },
    { field: 'status', headerName: t('status'), width: 110,
      renderCell: (params: any) => {
        const sc = (statusColors as any)[`surveyStatus_${params.value}`] || { bg: 'rgba(148,163,184,0.15)', color: '#94a3b8' };
      ...
      renderCell: (params: any) => (
        <Stack direction="row" spacing={0.5}>
          <Tooltip title={t('viewReadings')}>
            <IconButton size="small" onClick={() => handlePointClick(params.row)} sx={{ color: theme.palette.secondary.main, bgcolor: `${theme.palette.secondary.main}15` }}>
  ...
  const readingColumns = [
    { field: 'reading_date', headerName: t('readingDate'), width: 120, renderCell: (params: any) => params.value ? formatDate(params.value) : '' },
    { field: 'northing_reading', headerName: t('northing'), width: 110, renderCell: (params: any) => params.value ?? '-' },
    { field: 'easting_reading', headerName: t('easting'), width: 110, renderCell: (params: any) => params.value ?? '-' },
    { field: 'elevation_reading', headerName: t('elevation'), width: 110, renderCell: (params: any) => params.value ?? '-' },
    ...
    { field: 'actions', headerName: t('actions'), width: 120, sortable: false,
      renderCell: (params: any) => (
        <Stack direction="row" spacing={0.5}>
          <IconButton size="small" onClick={() => { setEditReading(params.row); setReadingFormOpen(true); }} sx={{ color: theme.palette.secondary.main, bgcolor: `${theme.palette.secondary.main}15` }}>
            <Edit fontSize="small" />
          </IconButton>
  ...
  const projectSelector = (
    <Stack direction="row" spacing={1.5} mb={2.5}>
      <TextField select size="small" value={selectedProjectId} onChange={(e: any) => { setSelectedProjectId(e.target.value); setSelectedPoint(null); setReadings([]); const p = new URLSearchParams(searchParams as any); p.set('project_id', e.target.value); setSearchParams(p, { replace: true }); }}
        sx={{ minWidth: 240 }}>
  ...
  return (
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }}>
      ...
      <Card sx={{ borderTop: '4px solid', borderTopColor: 'secondary.main' }}>
        {loading ? (
          ...
          <>
            <Box sx={{ height: 400 }}>
              <DataGrid
                rows={points} columns={pointColumns} getRowId={(r: any) => r.id}
                pageSizeOptions={[10, 20, 50]} disableRowSelectionOnClick
                onRowClick={(params: any) => handlePointClick(params.row)}
                localeText={i18n.language === 'ar' ? arSD : enUS}
                sx={{
                  '& .MuiDataGrid-row': { cursor: 'pointer' },
                  '& .MuiDataGrid-row.selected': { bgcolor: 'action.selected' },
                }}
              />
            </Box>
              ...
              <Box sx={{ borderTop: '1px solid', borderColor: 'divider' }}>
                ...
                <Box sx={{ height: 260 }}>
                  {readingsLoading ? (
                    ...
                    <DataGrid
                      rows={readings} columns={readingColumns} getRowId={(r: any) => r.id}
      ...
      <Dialog open={formOpen} onClose={() => setFormOpen(false)} maxWidth="sm" fullWidth>
        <form onSubmit={(e: React.FormEvent) => { e.preventDefault(); const fd = new FormData(e.target as HTMLFormElement); const data: Record<string, any> = {}; pointFields.forEach(f => { data[f.name] = fd.get(f.name); }); handleSubmit(data); }}>
          <DialogTitle>{editItem ? `${t('edit')} ${t('surveyPoint')}` : `${t('create')} ${t('surveyPoint')}`}</DialogTitle>
          <DialogContent>
            <Stack spacing={2} sx={{ mt: 1 }}>
              {pointFields.map((f: any) => (
                f.options ? (
      ...
      <Dialog open={readingFormOpen} onClose={() => setReadingFormOpen(false)} maxWidth="sm" fullWidth>
        <form onSubmit={(e: React.FormEvent) => { e.preventDefault(); const fd = new FormData(e.target as HTMLFormElement); const data: Record<string, any> = {}; readingFields.forEach(f => { data[f.name] = fd.get(f.name); }); handleReadingSubmit(data); }}>
          <DialogTitle>{editReading ? `${t('edit')} ${t('reading')}` : `${t('create')} ${t('reading')}`}</DialogTitle>
          <DialogContent>
            <Stack spacing={2} sx={{ mt: 1 }}>
              {readingFields.map((f: any) => (
                <TextField key={f.name} name={f.name} label={f.label} type={f.type === 'date' ? 'date' : f.type === 'number' ? 'number' : 'text'}
