...
export default function Specifications() {
  ...
  const [sectionDialog, setSectionDialog] = useState<{ open: boolean; specId: number | null; sections: any[]; loading: boolean }>({ open: false, specId: null, sections: [], loading: false });
  ...
  const openSections = async (specId: number) => {
  ...
  const handleSectionSubmit = async (formData: Record<string, any>) => {
    try {
  ...
  const columns = [
    ...
    { field: 'status', headerName: t('status'), width: 110,
      renderCell: (params: any) => {
        const sc = (statusColors as any)[`specStatus_${params.value}`] || { bg: 'rgba(148,163,184,0.15)', color: '#94a3b8' };
    ...
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }}>
      ...
      <Stack direction="row" spacing={1.5} mb={2.5} alignItems="center">
        ...
        <TextField size="small" placeholder={t('searchByKeyword')} value={searchTerm}
          onChange={(e: any) => handleSearch(e.target.value)} sx={{ minWidth: 200 }} />
      ...
      <Dialog open={sectionDialog.open} onClose={() => setSectionDialog({ open: false, specId: null, sections: [] })} maxWidth="md" fullWidth>
        ...
        <DialogContent>
          {sectionDialog.loading ? (
            ...
            <Stack spacing={1.5}>
              {sectionDialog.sections.map((sec: any) => (
      ...
      <Dialog open={sectionFormOpen} onClose={() => { setSectionFormOpen(false); setEditSection(null); }} maxWidth="sm" fullWidth>
        <form onSubmit={(e: React.FormEvent) => { e.preventDefault(); const fd = new FormData(e.target as HTMLFormElement); const data = { section_number: fd.get('section_number'), title: fd.get('title'), content: fd.get('content') }; handleSectionSubmit(data); }}>
