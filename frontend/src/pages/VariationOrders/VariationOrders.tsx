...
import {
  ...
  TableRow, Paper, Divider, IconButton, Fade, Tabs, Tab, Grid, LinearProgress, useTheme, alpha,
} from '@mui/material';
    ...
    { field: 'approved_date', headerName: 'Approved', width: 110, renderCell: (p) => formatDate(p.value) },
    {
      ...
      renderCell: (p) => (
        <Stack direction="row" spacing={0.5}>
          <IconButton size="small" onClick={() => openImpact(p.row)} sx={{ color: theme.palette.secondary.main, backgroundColor: alpha(theme.palette.secondary.main, 0.08), '&:hover': { backgroundColor: alpha(theme.palette.secondary.main, 0.16) } }}>
            <Analytics fontSize="small" />
          </IconButton>
          <IconButton size="small" onClick={() => openEdit(p.row)} sx={{ color: theme.palette.secondary.main, backgroundColor: alpha(theme.palette.secondary.main, 0.08), '&:hover': { backgroundColor: alpha(theme.palette.secondary.main, 0.16) } }}>
            <Edit fontSize="small" />
          </IconButton>
    ...
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }}>
      <Box>
        ...
        <Dialog open={impactOpen} onClose={() => setImpactOpen(false)} maxWidth="lg" fullWidth TransitionComponent={Fade} transitionDuration={250}>
          ...
          <DialogContent>
            {impactSummary && (
              <Card sx={{ mb: 3, px: 2.5, py: 2, bgcolor: 'primary.main', color: 'primary.contrastText' }}>
