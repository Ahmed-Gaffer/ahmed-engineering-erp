...
const CHART_COLORS = ['#3b82f6', '#f59e0b', '#ef4444', '#000000'];
...
const KpiCard = ({ title, value, color, icon, subtitle }) => (
  <Card sx={{ height: '100%', position: 'relative', overflow: 'hidden' }}>
    <Box sx={{ position: 'absolute', top: 0, left: 0, right: 0, height: 3, background: `linear-gradient(90deg, ${color}, ${color}88)`, opacity: 0.7 }} />
    <CardContent>
      <Stack direction="row" justifyContent="space-between" alignItems="flex-start">
        ...
        {icon && (
          <Box sx={{ width: 40, height: 40, borderRadius: 2, display: 'flex', alignItems: 'center', justifyContent: 'center', background: `linear-gradient(135deg, ${color}15, ${color}08)`, color, backdropFilter: 'blur(8px)', border: `1px solid ${color}22` }}>
            {icon}
          </Box>
        )}
      </Stack>
              ...
              <ChartCard title={t('statusBreakdown')} subtitle={t('incidents')}>
                <Box sx={{ height: 280 }}>
                  {(data.by_status || []).length === 0 ? (
                    ...
                    <ResponsiveContainer width="100%" height={280}>
                      <BarChart data={(data.by_status || []).map(s => ({ name: t(s.status), value: s.count }))}>
                        <CartesianGrid strokeDasharray="3 3" />
                        <XAxis dataKey="name" tick={{ fontSize: 11 }} />
                        <YAxis tick={{ fontSize: 11 }} allowDecimals={false} />
                        <ReTooltip content={<CustomTooltip />} />
          ...
          <Grid container spacing={2.5}>
            <Grid item xs={12} md={6}>
              <Card sx={{ position: 'relative', overflow: 'hidden' }}>
                <Box sx={{ position: 'absolute', top: 0, left: 0, right: 0, height: 3, background: 'linear-gradient(90deg, #ef4444, #f87171)', opacity: 0.6 }} />
            ...
            <Grid item xs={12} md={6}>
              <Card sx={{ position: 'relative', overflow: 'hidden' }}>
                <Box sx={{ position: 'absolute', top: 0, left: 0, right: 0, height: 3, background: 'linear-gradient(90deg, #3b82f6, #60a5fa)', opacity: 0.6 }} />
                <CardContent sx={{ p: { xs: 2, md: 2.5 } }}>
                  <Stack direction="row" justifyContent="space-between" alignItems="center" mb={2}>
                    <Box>
