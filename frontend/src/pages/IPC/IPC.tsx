...
export default function IPC() {
  ...
  const [boqItems, setBoqItems] = useState<Array<Record<string, unknown>>>([]);
  ...
  const [selectedBoqItems, setSelectedBoqItems] = useState<Record<string, number | string>>({});
  const [search, setSearch] = useState('');
  const [filterModel, setFilterModel] = useState<{ items: Array<Record<string, unknown>> }>({ items: [] });
  ...
  const handleViewDetail = async (row: Record<string, unknown>) => {
    ...
    try {
      const res = await engineeringApi.ipcs.get(row.id as number);
  ...
  const handleApprove = (id: number) => handleWorkflowAction(id, 'approve');
  const handleSubmit = (id: number) => handleWorkflowAction(id, 'submit');
  const handleReject = (id: number) => handleWorkflowAction(id, 'reject');
  const handlePay = (id: number) => handleWorkflowAction(id, 'pay');

  const handleApprovalConfirm = async (comment?: string, assignedTo?: string) => {
    try {
      const params: Record<string, unknown> = {};
      if (comment) params.comment = comment;
      if (assignedTo) params.assigned_to = assignedTo;
      await engineeringApi.ipcs[approvalAction as 'approve' | 'submit' | 'reject' | 'pay'](approvalId!, params);
  ...
  const handleCreateFormChange = (field: string) => (e: React.ChangeEvent<HTMLInputElement>) => {
  ...
  const handleBoqItemToggle = (itemId: number) => {
  ...
  const handleBoqItemQtyChange = (itemId: number) => (e: React.ChangeEvent<HTMLInputElement>) => {
  ...
  const handleCreateSubmit = async () => {
    ...
    const details = Object.entries(selectedBoqItems)
      .filter(([_, qty]) => qty > 0)
      .map(([boq_item_id, current_quantity]) => ({ boq_item_id: Number(boq_item_id), current_quantity }));
    if (details.length === 0) {
      enqueueSnackbar(t('fieldRequired'), { variant: 'warning' });
      return;
    }
  ...
  const columns = [
    ...
    { field: 'status', headerName: t('status'), width: 130,
      renderCell: (params: { value: string }) => chipStatus(params.value),
    },
    { field: 'total_works', headerName: 'Works', type: 'number', width: 120,
      renderCell: (params: { value: number }) => <Typography variant="body2">{formatNumber(params.value)}</Typography>,
    },
    { field: 'gross_amount', headerName: 'Gross', type: 'number', width: 120,
      renderCell: (params: { value: number }) => <Typography variant="body2" fontWeight={500}>{formatNumber(params.value)}</Typography>,
    },
    { field: 'net_amount', headerName: t('netAmount'), type: 'number', width: 120,
      renderCell: (params: { value: number }) => <Typography variant="body2" fontWeight={600} color="primary.main">{formatNumber(params.value)}</Typography>,
    },
    { field: 'total_to_date', headerName: 'Total to Date', type: 'number', width: 130,
      renderCell: (params: { value: number }) => <Typography variant="body2" fontWeight={600} color="success.main">{formatNumber(params.value)}</Typography>,
    },
    {
      field: 'actions', headerName: t('actions'), width: 360, sortable: false,
      renderCell: (params: { row: Record<string, unknown> }) => (
        <Stack direction="row" spacing={0.5}>
          <IconButton size="small" onClick={() => handleViewDetail(params.row)} sx={{ color: theme.palette.secondary.main, backgroundColor: `${theme.palette.secondary.main}14`, '&:hover': { backgroundColor: `${theme.palette.secondary.main}29` } }}>
            <Visibility fontSize="small" />
          </IconButton>
          {params.row.status === 'draft' && (
            <>
              <IconButton size="small" onClick={() => handleSubmit(params.row.id as number)} sx={{ color: theme.palette.warning.main, backgroundColor: `${theme.palette.warning.main}14`, '&:hover': { backgroundColor: `${theme.palette.warning.main}29` } }}>
                <Send fontSize="small" />
              </IconButton>
              <IconButton size="small" onClick={() => handleApprove(params.row.id as number)} sx={{ color: theme.palette.success.main, backgroundColor: `${theme.palette.success.main}14`, '&:hover': { backgroundColor: `${theme.palette.success.main}29` } }}>
          ...
          {params.row.status === 'submitted' && (
            <>
              ...
              <IconButton size="small" onClick={() => handleReject(params.row.id as number)} sx={{ color: theme.palette.error.main, backgroundColor: `${theme.palette.error.main}14`, '&:hover': { backgroundColor: `${theme.palette.error.main}29` } }}>
                <CloseIcon fontSize="small" />
              </IconButton>
            </>
          )}
          {params.row.status === 'approved' && (
            <IconButton size="small" onClick={() => handlePay(params.row.id as number)} sx={{ color: theme.palette.secondary.main, backgroundColor: `${theme.palette.secondary.main}14`, '&:hover': { backgroundColor: `${theme.palette.secondary.main}29` } }}>
              <AttachMoney fontSize="small" />
            </IconButton>
          )}
          <IconButton size="small" onClick={() => handlePrint(params.row.id as number)} sx={{ color: theme.palette.secondary.main, backgroundColor: 'rgba(139,92,246,0.08)', '&:hover': { backgroundColor: 'rgba(139,92,246,0.16)' } }}>
            <Print fontSize="small" />
          </IconButton>
          <IconButton size="small" onClick={() => handleExportPdf(params.row.id as number)} sx={{ color: theme.palette.error.dark, backgroundColor: `${theme.palette.error.dark}14`, '&:hover': { backgroundColor: `${theme.palette.error.dark}29` } }}>
            <PictureAsPdf fontSize="small" />
          </IconButton>
          <IconButton size="small" onClick={() => handleExportExcel(params.row.id as number)} sx={{ color: theme.palette.secondary.main, backgroundColor: `${theme.palette.secondary.main}14`, '&:hover': { backgroundColor: `${theme.palette.secondary.main}29` } }}>
            <Download fontSize="small" />
          </IconButton>
          {params.row.status === 'draft' && (
            <IconButton size="small" onClick={() => { setDeleteId(params.row.id as number); setDeleteOpen(true); }} sx={{ color: theme.palette.error.main, backgroundColor: `${theme.palette.error.main}14`, '&:hover': { backgroundColor: `${theme.palette.error.main}29` } }}>
  ...
  return (
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ duration: 0.2 }}>
      <Box>
        ...
        <Card sx={{ mb: 3, px: 2.5, py: 2, borderTop: '3px solid', borderTopColor: 'secondary.main', borderTopLeftRadius: 0, borderTopRightRadius: 0 }}>
          <Stack direction="row" alignItems="center" spacing={2} flexWrap="wrap" gap={1.5}>
            ...
            >
              ...
              {projects.map((p) => (
                <MenuItem key={p.id as number} value={p.id as number}>{p.name || p.code as string}</MenuItem>
              ))}
            </TextField>
            {selectedProjectId && (
              <TextField
        ...
        {summary && (
          <Card sx={{ mb: 3, px: 2.5, py: 2, bgcolor: 'primary.main', color: 'primary.contrastText', borderTop: '3px solid', borderTopColor: 'secondary.main', borderTopLeftRadius: 0, borderTopRightRadius: 0 }}>
            <Grid container spacing={3} alignItems="center">
              <Grid item xs={12} sm={6} md={2}>
                <Typography variant="caption" color="rgba(255,255,255,0.7)">Total IPCs</Typography>
                <Typography variant="h5" color="white" fontWeight={700}>{summary.total_ipcs as number}</Typography>
              </Grid>
              <Grid item xs={12} sm={6} md={2}>
                <Typography variant="caption" color="rgba(255,255,255,0.7)">Total Billed</Typography>
                <Typography variant="h6" color="white" fontWeight={600}>{formatNumber(summary.total_billed as number)}</Typography>
              </Grid>
              <Grid item xs={12} sm={6} md={2}>
                <Typography variant="caption" color="rgba(255,255,255,0.7)">Total Paid</Typography>
                <Typography variant="h6" color="#34d399" fontWeight={600}>{formatNumber(summary.total_paid as number)}</Typography>
              </Grid>
              <Grid item xs={12} sm={6} md={2}>
                <Typography variant="caption" color="rgba(255,255,255,0.7)">Contract Ceiling</Typography>
                <Typography variant="h6" color="white" fontWeight={600}>{formatNumber(summary.contract_ceiling as number)}</Typography>
              </Grid>
              <Grid item xs={12} sm={6} md={2}>
                <Typography variant="caption" color="rgba(255,255,255,0.7)">Remaining</Typography>
                <Typography variant="h6" color="#fbbf24" fontWeight={600}>{formatNumber(summary.remaining as number)}</Typography>
              </Grid>
              <Grid item xs={12} sm={6} md={2}>
                <Typography variant="caption" color="rgba(255,255,255,0.7)">Progress</Typography>
                <Typography variant="h6" color="white" fontWeight={700}>{formatNumber(summary.progress_percent as number)}%</Typography>
        ...
        >
          ...
          <Divider sx={{ my: 1.5 }} />
          <DialogContent>
            <Tabs value={detailTab} onChange={(_, v: number) => setDetailTab(v)} sx={{ mb: 2 }} variant="scrollable">
              <Tab label="Summary" />
              ...
              {detailItem && (
                <Grid container spacing={2}>
                  <Grid item xs={12} sm={6}>
                    ...
                    <Typography variant="body1" fontWeight={600}>{chipStatus(detailItem.status as string)}</Typography>
                  </Grid>
                  <Grid item xs={12} sm={6}>
                    <Typography variant="caption" color="text.secondary">Period</Typography>
                    <Typography variant="body1">{detailItem.start_date && formatDate(detailItem.start_date as string)} to {detailItem.end_date && formatDate(detailItem.end_date as string)}</Typography>
                  </Grid>
                  <Grid item xs={12} sm={4}>
                    <Typography variant="caption" color="text.secondary">{t('totalWorks')}</Typography>
                    <Typography variant="h6">{formatNumber(detailItem.total_works as number)}</Typography>
                  </Grid>
                  <Grid item xs={12} sm={4}>
                    <Typography variant="caption" color="text.secondary">{t('materialsOnSite')}</Typography>
                    <Typography variant="h6">{formatNumber(detailItem.materials_on_site as number)}</Typography>
                  </Grid>
                  <Grid item xs={12} sm={4}>
                    <Typography variant="caption" color="text.secondary">{t('grossAmount')}</Typography>
                    <Typography variant="h6" color="primary.main" fontWeight={700}>{formatNumber(detailItem.gross_amount as number)}</Typography>
                  </Grid>
                  <Grid item xs={12} sm={4}>
                    <Typography variant="caption" color="text.secondary">{t('totalDeductions')}</Typography>
                    <Typography variant="h6" color="error.main">{formatNumber(detailItem.total_deductions as number)}</Typography>
                  </Grid>
                  <Grid item xs={12} sm={4}>
                    <Typography variant="caption" color="text.secondary">Net Amount</Typography>
                    <Typography variant="h6" color="success.main" fontWeight={700}>{formatNumber(detailItem.net_amount as number)}</Typography>
                  </Grid>
                  <Grid item xs={12} sm={4}>
                    <Typography variant="caption" color="text.secondary">{t('totalToDate')}</Typography>
                    <Typography variant="h6" color="info.main" fontWeight={700}>{formatNumber(detailItem.total_to_date as number)}</Typography>
                  </Grid>
                  <Grid item xs={12}>
                    <Divider sx={{ my: 1 }} />
                    <Stack direction="row" spacing={4} flexWrap="wrap">
                      <Box><Typography variant="caption" color="text.secondary">{t('contractCeiling')}</Typography><Typography>{formatNumber(detailItem.contract_ceiling as number)}</Typography></Box>
                      <Box><Typography variant="caption" color="text.secondary">{t('totalBilled')}</Typography><Typography>{formatNumber(detailItem.total_billed as number)}</Typography></Box>
                      <Box><Typography variant="caption" color="text.secondary">Remaining</Typography><Typography color="warning.main">{formatNumber((detailItem.contract_ceiling as number) - (detailItem.total_billed as number))}</Typography></Box>
                      <Box><Typography variant="caption" color="text.secondary">{t('previousTotal')}</Typography><Typography>{formatNumber(detailItem.previous_total as number)}</Typography></Box>
                    </Stack>
                  </Grid>
                </Grid>
              )}
            ...
            <TabPanel value={detailTab} index={1}>
              {detailLoading ? (
              ...
              ) : !detailItem?.details || (detailItem.details as Array<unknown>).length === 0 ? (
                ...
                <TableContainer component={Paper} variant="outlined">
                  <Table size="small">
                    ...
                    <TableBody>
                      {(detailItem.details as Array<Record<string, unknown>>).map((item) => (
              ...
              {detailItem && (
                <Grid container spacing={2}>
                  <Grid item xs={12} sm={6}>
                    ...
                    <Stack spacing={1}>
                      <Stack direction="row" justifyContent="space-between"><Typography>{t('retention')} ({detailItem.retention_percent}%)</Typography><Typography>{formatNumber(detailItem.retention_amount as number)}</Typography></Stack>
                      <Stack direction="row" justifyContent="space-between"><Typography>{t('advanceRecovery')}</Typography><Typography>{formatNumber(detailItem.advance_recovery as number)}</Typography></Stack>
                      <Stack direction="row" justifyContent="space-between"><Typography>{t('fines')}</Typography><Typography color="error.main">{formatNumber(detailItem.fines as number)}</Typography></Stack>
                      <Stack direction="row" justifyContent="space-between"><Typography>{t('insurance')}</Typography><Typography>{formatNumber(detailItem.insurance as number)}</Typography></Stack>
                      <Stack direction="row" justifyContent="space-between"><Typography>{t('otherDeductions')}</Typography><Typography>{formatNumber(detailItem.other_deductions as number)}</Typography></Stack>
                      <Divider />
                      <Stack direction="row" justifyContent="space-between"><Typography fontWeight={700}>{t('totalDeductions')}</Typography><Typography fontWeight={700} color="error.main">{formatNumber(detailItem.total_deductions as number)}</Typography></Stack>
                    </Stack>
                  </Grid>
                  <Grid item xs={12} sm={6}>
                    <Typography variant="subtitle2" fontWeight={600} color="success.main" gutterBottom>Cumulative Summary</Typography>
                    <Stack spacing={1}>
                      <Stack direction="row" justifyContent="space-between"><Typography>{t('previousTotal')}</Typography><Typography>{formatNumber(detailItem.previous_total as number)}</Typography></Stack>
                      <Stack direction="row" justifyContent="space-between"><Typography fontWeight={600}>{t('currentDue')}</Typography><Typography fontWeight={600} color="primary.main">{formatNumber(detailItem.current_due as number)}</Typography></Stack>
                      <Divider />
                      <Stack direction="row" justifyContent="space-between"><Typography fontWeight={700}>{t('totalToDate')}</Typography><Typography fontWeight={700} color="success.main">{formatNumber(detailItem.total_to_date as number)}</Typography></Stack>
                      <Divider />
                      <Stack direction="row" justifyContent="space-between"><Typography>{t('contractCeiling')}</Typography><Typography>{formatNumber(detailItem.contract_ceiling as number)}</Typography></Stack>
                      <Stack direction="row" justifyContent="space-between"><Typography>Remaining</Typography><Typography color="warning.main">{formatNumber((detailItem.contract_ceiling as number) - (detailItem.total_billed as number))}</Typography></Stack>
                    </Stack>
                  </Grid>
                </Grid>
              )}
            ...
            <TabPanel value={detailTab} index={3}>
              <WorkflowTimeline entityType="ipc" entityId={detailItem?.id as number} refreshTrigger={wfRefresh} />
            </TabPanel>
          </DialogContent>
          <Divider />
          <DialogActions sx={{ px: 3, py: 1.5 }}>
            <Button onClick={() => handleExportPdf(detailItem?.id as number)} size="small" startIcon={<PictureAsPdf />} color="error">PDF</Button>
            <Button onClick={() => handlePrint(detailItem?.id as number)} size="small" startIcon={<Print />}>{t('printIpc')}</Button>
        ...
        <ApprovalDialog
          ...
          currentStatus={detailItem?.status as string || ''}
          ...
          <DialogContent>
            <Stack spacing={2} mb={3}>
              ...
              >
                ...
                {contracts.map((c) => (
                  <MenuItem key={c.id as number} value={c.id as number}>{c.name || c.contract_number as string}</MenuItem>
                ))}
              </TextField>
                ...
                {boqItems.length === 0 ? (
                  ...
                  <TableContainer component={Paper} variant="outlined" sx={{ maxHeight: 300 }}>
                    <Table size="small" stickyHeader>
                      ...
                      <TableBody>
                        {boqItems.map((item) => (
                          <TableRow key={item.id as number} hover selected={item.id as number in selectedBoqItems}>
                            <TableCell padding="checkbox">
                              <Checkbox
                                checked={item.id as number in selectedBoqItems}
                                onChange={() => handleBoqItemToggle(item.id as number)}
                              />
                            </TableCell>
                            ...
                            <TableCell align="right" sx={{ maxWidth: 120 }}>
                              {item.id as number in selectedBoqItems ? (
                                <TextField
                                  type="number" size="small" autoFocus
                                  value={selectedBoqItems[item.id as number]}
                                  onChange={handleBoqItemQtyChange(item.id as number)}
