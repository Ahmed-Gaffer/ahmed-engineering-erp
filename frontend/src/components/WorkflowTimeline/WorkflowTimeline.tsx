...
const ACTION_COLORS: Record<string, { bg: string; icon: string; border: string }> = {
...
export default function WorkflowTimeline({ entityType, entityId, refreshTrigger }: WorkflowTimelineProps) {
  ...
  useEffect(() => {
    ...
    workflowApi.getLogs(entityType, entityId)
      .then((r: { data?: WorkflowLog[] }) => setLogs(r.data || []))
      .catch(() => setLogs([]))
